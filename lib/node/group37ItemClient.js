...
export const group37ItemClient = {
  ...
  async createItem(groupId, { createdAt, sourceId, sourceSortKey }) {
    ...
    if (sourceObj) {
      ...
      if (groupId === noGroupSortKey) {
        await asyncForAll(currentGroups, async group => {
  ...
  async createItems(groupIds, group37Prefix, { createdAt, sourceId, sourceSortKey }) {
    if (groupIds?.length) {
      const items = [];
      await asyncForAll(groupIds, async groupId => {
        const { item } = await group37ItemClient.createItem(groupId, {
          createdAt: createdAt,
          sourceId: sourceId,
          sourceSortKey: sourceSortKey,
        });
        items.push({ id: item.id, itemId: item.sortKey });
      });

      return items;
    } else {
      const { item } = await group37ItemClient.createItem(getNoGroupSortKey(group37Prefix), {
        createdAt: createdAt,
        sourceId: sourceId,
        sourceSortKey: sourceSortKey,
      });
      return [{ id: item.id, itemId: item.sortKey }];
    }
  },
