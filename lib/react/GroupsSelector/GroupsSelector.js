...
function GroupsSelector({
  ...
  return (
    <>
      <Text weight="bold" onClick={() => setShowSelector(!showSelector)}>
        Select tags
      </Text>
      {!!selectedGroups?.length && (
        <Box direction="row" wrap onClick={() => setShowSelector(!showSelector)}>
          {(selectedGroups || []).map(
            id => groupsObj[id] && <Text key={id} margin="0 0.5rem 0 0">#{groupsObj[id].title}</Text>
          )}
        </Box>
      )}
