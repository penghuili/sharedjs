...
function* handlePayPressed({ payload: { app, code } }) {
  yield put(sharedActionCreators.isPaying(true));
  yield put(sharedActionCreators.setPayError(null));

  const { data, error } = yield call(pay, app, code);
  ...
  if (data) {
    ...
    yield put(sharedActionCreators.paySucceeded(app));
  } else {
    if (httpErrorCodes.NOT_FOUND === error?.errorCode) {
      yield put(sharedActionCreators.setPayError('Invalid code.'));
    } else if (error?.errorCode === httpErrorCodes.INVALID_TICKET) {
      yield put(sharedActionCreators.setPayError('This code is already used.'));
    } else {
      yield put(sharedActionCreators.setPayError('Something went wrong, please try again.'));
    }
  }

  yield put(sharedActionCreators.isPaying(false));
}
...
export function* sharedSagas() {
  ...
  yield all([
    ...
    takeLatest(sharedActionTypes.PAY_PRESSED, handlePayPressed),
  ]);
}
