...
import { isValidEmail, isValidUsername } from '../../js/regex';
...
function* hanldeSignUpPressed({ payload: { username, email, password } }) {
  ...
  if (email && !isValidEmail(email)) {
    yield put(sharedActionCreators.setAuthError('Please you a valid email.'));
    return;
  }

  yield put(sharedActionCreators.isLoadingAuth(true));
  const { error } = yield call(signUp, username, email, password);

  if (error) {
    if (error.errorCode === httpErrorCodes.NO_USERNAME_OR_EMAIL) {
      yield put(
        sharedActionCreators.setAuthError(
          username ? 'Please enter your username.' : 'Please enter your email.'
        )
      );
    } else if (error.errorCode === httpErrorCodes.ALREADY_EXISTS) {
      yield put(
        sharedActionCreators.setAuthError(
          username ? 'This username is used.' : 'This email is used.'
        )
      );
...
function* hanldeSignInPressed({ payload: { username, email, password } }) {
  yield put(sharedActionCreators.isLoadingAuth(true));

  const { data, error } = yield call(signIn, username, email, password);
  if (error) {
    if (error.errorCode === httpErrorCodes.NOT_FOUND) {
      yield put(sharedActionCreators.setAuthError('This user does not exist.'));
    } else {
...
function* handleChangePasswordPressed({ payload: { currentPassword, newPassword } }) {
  ...
  const { username, email } = yield select(sharedSelectors.getAccount);
  const { data } = yield call(changePassword, username, email, currentPassword, newPassword);
