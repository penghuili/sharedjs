...
export async function fetchFiles(startKey, groupId) {
  try {
    const startKeyQuery = startKey ? `startKey=${startKey}` : '';
    const groupIdQuery = groupId ? `groupId=${groupId}` : '';
    const queryString = [startKeyQuery, groupIdQuery].filter(q => q).join('&');
    const {
      items,
...
async function decryptFileContent(file) {
  ...
  return { ...file, fileName: decryptedFileName, note: decryptedNote };
}
