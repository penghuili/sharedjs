export const sharedActionTypes = {
  ...
  PAY_PRESSED: 'shared/PAY_PRESSED',
  IS_PAYING: 'shared/IS_PAYING',
  PAY_SUCCEEDED: 'shared/PAY_SUCCEEDED',
  IS_LOADING_SETTINGS: 'shared/IS_LOADING_SETTINGS',
  SET_SETTINGS: 'shared/SET_SETTINGS',
  SET_PAY_ERROR: 'shared/SET_PAY_ERROR',
};
...
export const sharedActionCreators = {
  ...
  setUserData({ userId, username, twoFactorEnabled, twoFactorUri, createdAt, botPublicKey }) {
    return {
      type: sharedActionTypes.SET_USER_DATA,
      payload: { userId, username, twoFactorEnabled, twoFactorUri, createdAt, botPublicKey },
    };
  },
  ...
  fetchSettingsRequested(app) {
    return { type: sharedActionTypes.FETCH_SETTINGS_REQUESTED, payload: { app } };
  },
  ...
  payPressed(app, code) {
    return {
      type: sharedActionTypes.PAY_PRESSED,
      payload: { app, code },
    };
  },
  isPaying(value) {
    return {
      type: sharedActionTypes.IS_PAYING,
      payload: { value },
    };
  },
  paySucceeded(app) {
    return {
      type: sharedActionTypes.PAY_SUCCEEDED,
      payload: { app },
    };
  },
  ...
  setPayError(message) {
    return { type: sharedActionTypes.SET_PAY_ERROR, payload: { message } };
  },
};
