...
const initialState = {
  themeMode: LocalStorage.get(sharedLocalStorageKeys.themeMode) || 'light',
  toast: null,
...
function handleSetUserData(
  state,
  { userId, username, twoFactorChecked, twoFactorEnabled, twoFactorUri, createdAt, botPublicKey }
) {
  return {
    ...state,
    userId,
...
function handleIsDeletingAccount(state, { value }) {
  return { ...state, isDeletingAccount: value };
}
...
export function sharedReducer(state = initialState, action) {
  switch (action.type) {
    ...
    case sharedActionTypes.IS_DELETING_ACCOUNT:
      return handleIsDeletingAccount(state, action.payload);
