...
function handleIsPaying(state, { value }) {
  return { ...state, isPaying: value };
}
...
export function sharedReducer(state = initialState, action) {
  switch (action.type) {
    ...
    case sharedActionTypes.SET_PAY_ERROR:
    ...
    case sharedActionTypes.IS_PAYING:
      return handleIsPaying(state, action.payload);
