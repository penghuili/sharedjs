...
export function createGeneralStore(
  ...
  const {
    ...
    onReducerSucceeded: (state, { data, payload: { id, itemId, reorder } }) => {
      const safeId = id || defaultId;
      const newState = updateBySortKey(state, ['data', safeId, 'items'], itemId, data);
      if (reorder) {
  ...
  const {
    ...
    onReducerSucceeded: (state, { payload: { id, itemId } }) => {
      const safeId = id || defaultId;
      return removeBySortKey(state, ['data', safeId, 'items'], itemId);
    },
