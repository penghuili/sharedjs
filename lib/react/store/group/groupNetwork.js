...
export async function fetchGroups(prefix) {
  try {
    const groups = await HTTP.get(apps.Group37.name, `/v1/groups?prefix=${prefix}`);
...
export async function fetchGroup(groupId) {
  try {
    const group = await HTTP.get(apps.Group37.name, `/v1/groups/${groupId}`);
...
export async function createGroup(title, sortKeyPrefix, isNoGroup) {
  try {
    ...
    const group = await HTTP.post(apps.Group37.name, `/v1/groups`, {
...
export async function updateGroup(groupId, { title, position, isSecondary }, decryptedPassword) {
  try {
    ...
    const group = await HTTP.put(apps.Group37.name, `/v1/groups/${groupId}`, {
      title: encryptedTitle,
      position,
      isSecondary,
    });
...
export async function deleteGroup(groupId) {
  try {
    const result = await HTTP.delete(apps.Group37.name, `/v1/groups/${groupId}`);
...
export async function createGroupItem(groupId, { createdAt, sourceId, sourceSortKey }) {
  try {
    const item = await HTTP.post(apps.Group37.name, `/v1/groups/${groupId}/items`, {
      createdAt,
      sourceId,
      sourceSortKey,
    });
...
export async function deleteGroupItem(groupId, itemId) {
  try {
    const result = await HTTP.delete(apps.Group37.name, `/v1/groups/${groupId}/items/${itemId}`);
