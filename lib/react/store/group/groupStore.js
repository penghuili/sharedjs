...
export function createGroupStore(domain) {
  ...
  const { actions, selectors, reducer, saga } = createGeneralStore(domain, {
    ...
    preUpdateItem: function* () {
      const group = yield select(dataSelectors.getStandaloneItem);
      return { continueCall: !!group, result: group };
    },
    updateItem: async ({ itemId, title, position }, group) => {
      return updateGroup(itemId, { title, position }, group.decryptedPassword);
    },
