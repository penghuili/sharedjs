...
function groupItems(state, sortKeyPrefix) {
  ...
  groupsObj = items.reduce((acc, group) => {
...
export function createGroupStore(domain, sortKeyPrefix) {
  ...
  const { actions, selectors, reducer, saga } = createGeneralStore(domain, {
    preFetchItems: function* (payload) {
      ...
      if (cachedGroups?.length) {
        yield put(actions.fetchItems.succeeded.action({ payload, data: { items: cachedGroups } }));
      }
    ...
    onFetchItemsSucceeded: state => {
      const grouped = groupItems(state, sortKeyPrefix);
      return grouped;
    },
  ...
  const groupSelectors = {
    ...
    data: {
      ...dataSelectors,
      getPrimaryItems: state => safeGet(state, [domain, defaultId, 'data', 'primaryItems'], []),
      getSecondaryItems: state => safeGet(state, [domain, defaultId, 'data', 'secondaryItems'], []),
      getGroupsObj: state => safeGet(state, [domain, defaultId, 'data', 'groupsObj']),
      getNoGroupItem: state => safeGet(state, [domain, defaultId, 'data', 'noGroupItem']),
    },
  };
