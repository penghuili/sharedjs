...
export async function fetchAccount() {
  try {
    const { id, username, backendPublicKey, twoFactorEnabled, twoFactorUri, createdAt, updatedAt } =
      await HTTP.get(apps.auth, `/v1/me`);

    const decryptedTwoFactorUri = twoFactorUri
      ? await decryptMessage(LocalStorage.get(sharedLocalStorageKeys.privateKey), twoFactorUri)
      : twoFactorUri;
...
export async function deleteAccount() {
  try {
    ...
    try {
      await HTTP.delete(apps.file37.name, `/v1/me`);
    } catch (e) {
      console.log(e);
    }
